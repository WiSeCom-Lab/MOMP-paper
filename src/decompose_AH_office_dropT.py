...
for method, K_res, N_UE, N_AP in cases:
    # Params
    ...
    if link == "up":
        ...
        A = L_invW_x_U.reshape((-1, 1, N_AP, N_AP, 1, 1)) * F_x_U.reshape((1, -1, 1, 1, N_UE, N_UE))
        A = A.reshape((-1, N_AP, N_AP, N_UE, N_UE))
    else:
        L_invW = []
        ...
        A = L_invW_x_U.reshape((-1, 1, N_UE, N_UE, 1, 1)) * F_x_U.reshape((1, -1, 1, 1, N_AP, N_AP))
        A = A.reshape((-1, N_UE, N_UE, N_AP, N_AP))
    ...
    for rays, ue_pos, ap, ii_ue in zip(Rays, UE_pos, AP_selected, range(len(UE_pos))):
        ...
        I, alpha = alg(M_U.reshape([-1, E_conv_U.shape[1]]))
        toc = time()-tic
        if method == "OMP":
    ...
    with open(
        "data/{}/AH/paths-dropT/single_{}_{}_{}_{}_{}_{}.json".format(
            set, method, N_M_UE, N_M_AP, Q, p_t_dBm, int(10*K_res)),
        "w") as f:
